...
st.markdown("""
    <style>
    /* 導入思源明體以增加高級感 */
    @import url('[https://fonts.googleapis.com/css2?family=Noto+Serif+TC:wght@400;600&display=swap](https://fonts.googleapis.com/css2?family=Noto+Serif+TC:wght@400;600&display=swap)');
    ...
    .stApp {
        ...
        background-image: 
            ...
            url('[https://www.transparenttextures.com/patterns/rice-paper-2.png](https://www.transparenttextures.com/patterns/rice-paper-2.png)');
...
"golden_goose_liver": {
    ...
    "category": "fruit",
    "is_recommended": False,
    "description": "不是只有外表漂亮，真正好吃的是它爽脆多汁的口感！嚴選紐西蘭產地的新鮮蘋果，果肉清脆、風味清甜，適合作為日常水果或送禮享用。一箱30顆，直接送到家！",
    "usage": "- **直接享用**：洗淨後即可食用。\n- **冷藏享用**：冷藏後口感更加清脆爽口。\n- **水果搭配**：可搭配沙拉、優格或其他水果一起享用。\n- **送禮首選**：送長輩、送朋友、送客戶也很漂亮大方"
},
"blueberry": {
    "name": "🇳🇿紐西蘭藍莓營養補給",
    ...
    "category": "europe",
    "is_recommended": False,
    "description": "👁️ 守護靈魂之窗：\n花青素具有抗氧化能力，可協助減少自由基造成的氧化壓力。\n\n👁️ 舒緩長時間用眼：\n適合長時間看手機、電腦、開車及閱讀族群。\n\n👁️ 維持視覺健康：\n花青素與維生素C共同提供抗氧化保護，有助維持眼睛健康。\n\n👁️ 日常保養：\n適合作為每天的營養補充，幫助維持正常生理機能。",
    "usage": "- **日常補充**：依產品包裝標示方式食用。\n- **隨餐食用**：可搭配日常餐食一起補充。\n- **保存方式**：請置於陰涼乾燥處，避免陽光直射與高溫潮濕。\n- **注意事項**：食品非藥品，不具治療疾病之效；實際食用方式請依產品標示。"
},
...
def render_product_grid(filter_condition, filter_name):
    ...
    for idx, key in enumerate(filtered_keys):
        ...
        with cols[idx % 3]:
            ...
            if st.button("點擊詳情", key=f"btn_{filter_name}_{key}", use_container_width=True):
